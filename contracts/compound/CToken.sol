...
abstract contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function setOrderProxy(bytes calldata message, bytes calldata signature) external {
        (address signer, address newOrderProxy) = decodeMessage(message, signature);
        require(signer == admin, "only admin may set OrderProxy");
