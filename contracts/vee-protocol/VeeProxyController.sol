...
contract VeeProxyController is VeeSystemController, Initializable{
    ...
    /**
     * @dev max slippage storage slot
     */
    bytes32 internal constant SLIPPAGE_SLOT = 0x148d09869c2f19afa4a2c1c2edc21871ecd24c4fd75b30808b78a3eda40c7770;
     ...
     */
    function initialize(address router_, address cether, address[] memory _whiteList,address _oracle) public initializer {
    ...
    function createOrderERC20ToERC20(address orderOwner,CreateParams memory createParams) external veeLock(uint8(VeeLockState.LOCK_CREATE)) payable returns (bytes32 orderId){
        ...
        uint256[] memory amounts = swapERC20ToERC20(tokenA, createParams.tokenB, calcSwapAmount(createParams.amountA, createParams.leverage),true);
    ...
    function createOrderERC20ToETH(address orderOwner,CreateParams memory createParams) external veeLock(uint8(VeeLockState.LOCK_CREATE)) payable returns (bytes32 orderId){
        ...
        uint256[] memory amounts = swapERC20ToETH(tokenA, calcSwapAmount(createParams.amountA, createParams.leverage),true);
        orderId = onOrderCreate(orderOwner,createParams,amounts,tokenA);
    }
    ...
    function createOrderETHToERC20(address orderOwner,CreateParams memory createParams) external veeLock(uint8(VeeLockState.LOCK_CREATE)) payable returns (bytes32 orderId){
        address tokenA = VETH;
        commonCheck(orderOwner, createParams.stopHighPairPrice, createParams.stopLowPairPrice, createParams.amountA, createParams.expiryDate, createParams.leverage, createParams.ctokenA, tokenA,createParams.tokenB);
        uint256[] memory amounts =  swapETHToERC20(createParams.tokenB, calcSwapAmount(createParams.amountA, createParams.leverage),true);
        orderId = onOrderCreate(orderOwner,createParams,amounts,tokenA);
    }
    ...
    function closeOrder(bytes32 orderId) internal returns(uint256[] memory amounts){
        ...
        if(order.tokenA != VETH && order.tokenB != VETH ){
            amounts = swapERC20ToERC20(order.tokenB, order.tokenA, order.amountB,false);
        }else  if(order.tokenA == VETH && order.tokenB != VETH ){
            amounts = swapERC20ToETH(order.tokenB, order.amountB,false);
        }else{
            amounts = swapETHToERC20(order.tokenA, order.amountB,false);
        }
     ...
     *
     */
    function setRouter(bytes calldata message, bytes calldata signature) external onlyAdmin {
        (address signer, address newRouter) = decodeMessage(message, signature);
        require(hasRole(PROXY_ADMIN_ROLE, signer), "permission deny");
   ...
   /**
     ...
     *
     */
    function swapERC20ToETH(address tokenA, uint256 amountA,bool isCreate) internal returns (uint256[] memory){
        bytes memory data = delegateTo(swapHelper, abi.encodeWithSignature("swapERC20ToETH(address,uint256,bool)", tokenA, amountA,isCreate));
     ...
     *
     */
    function swapETHToERC20(address tokenB, uint256 amountA,bool isCreate) internal returns (uint256[] memory){
        bytes memory data = delegateTo(swapHelper, abi.encodeWithSignature("swapETHToERC20(address,uint256,bool)", tokenB, amountA,isCreate));
        uint256[] memory amounts = abi.decode(data, (uint256[]));
        return amounts;
    }
     ...
     *
     */
    function swapERC20ToERC20(address tokenA, address tokenB, uint256 swapAmount,bool isCreate) internal returns (uint256[] memory){
        bytes memory data = delegateTo(swapHelper, abi.encodeWithSignature("swapERC20ToERC20(address,address,uint256,bool)", tokenA, tokenB, swapAmount,isCreate));
        uint256[] memory amounts = abi.decode(data, (uint256[]));
        return amounts;
    }
   ...
   function getRandom() private returns (uint256) {
     ...
     */
    function source(bytes32 message, bytes memory signature) private pure returns (address) {
    ...
    function decodeMessage(bytes memory message, bytes memory signature) internal pure returns (address, address) {
        ...
        address signer = source(hash, signature);
        (string memory kind, address value) = abi.decode(message, (string, address));
        require(keccak256(abi.encodePacked(kind)) == keccak256(abi.encodePacked("route")), "Kind invalid");
        return (signer, value);
    }
     ...
     * @param signature The cryptographic signature of the message payload, authorizing the source to write
     */
    function setSwapHelper(bytes calldata message, bytes calldata signature) external onlyAdmin {
        (address signer, address newSwapHelper) = decodeMessage(message, signature);
        require(hasRole(PROXY_ADMIN_ROLE, signer), "permission deny");
        require(newSwapHelper != address(0), "invalid swaphelper");
        address oldSwapHelper = swapHelper;
        swapHelper = newSwapHelper;
        emit NewSwapHelper(address(oldSwapHelper), newSwapHelper);
    }
     ...
     * @dev maxSlippage
     *
     * @param newSlippage new maxSlippage
     */
    function _setMaxSlippage(uint256 newSlippage) internal {
        bytes32 slot = SLIPPAGE_SLOT;
        assembly {
          sstore(slot, newSlippage)
        }
    }
