...
contract SwapHelper {
     ...
     */
    function swapERC20ToETH(address tokenA, uint256 amountA,bool isCreate) external payable returns (uint256[] memory){
        ...
        uint256 amountOutMin = getMinAmountOut(tokenA,path[1],amountA,isCreate);
     ...
     * @return pair reserveIn and reserveOut
     */
    function swapETHToERC20(address tokenB, uint256 amountA,bool isCreate) external payable returns (uint256[] memory){
        ...
        uint256 deadline = (block.timestamp + 99999999);
        uint256 amountOutMin = getMinAmountOut(path[0],tokenB,amountA,isCreate);
     ...
     * @return pair reserveIn and reserveOut
     */
    function swapERC20ToERC20(address tokenA, address tokenB, uint256 amountA,bool isCreate) external payable returns (uint256[] memory){
        require(tokenA != address(0), "invalid token A");
        require(tokenB != address(0), "invalid token B");
        ...
        uint256 deadline = (block.timestamp + 99999999);
        uint256[] memory amounts;
        uint256 amountOutMin = getMinAmountOut(tokenA,tokenB,amountA,isCreate);
        amounts = IPangolinRouter(router).swapExactTokensForTokens(amountA, amountOutMin, path, address(this), deadline);
        return amounts;
    }
     ...
     * @return maxSlippage
     */
    function _maxSlippage() internal view returns(uint256 maxSlippage) {
        bytes32 slot = SLIPPAGE_SLOT;
        assembly {
          maxSlippage := sload(slot)
        }
    }

        /**
     * @dev get minimal amountOut for trading
     *
     * @param tokenA tokenA address
     * @param tokenB tokenB address
     * @param amountA amount of tokenA
     ...
     * @return minAmountOut
     */
    function getMinAmountOut(address tokenA, address tokenB, uint amountA,bool isCreate) internal view returns (uint256 minAmountOut) {
        require(amountA != 0, "amountA can't be zero");
        if(isCreate){
            uint256 amountOut = getAmountOut(tokenA, tokenB, amountA);
            uint256 maxSlippage = _maxSlippage();
            minAmountOut = amountOut - ((amountOut * maxSlippage) / 10**18);        
        }else{
            minAmountOut = 1;
        }
        ...
        returns (uint256)
    {
        ...
        // uint256  price = flows[0].preciseDiv(sellDecimals).preciseDiv(flows[1].preciseDiv(buyDecimals));
